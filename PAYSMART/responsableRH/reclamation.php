...
?>
    ...
    if (isset($_POST['valider']) || isset($_POST['refuser'])) {
        ...
        if (!empty($commentaire)) {
            $updateQuery = "UPDATE `reclamation` SET commentaire = '$commentaire' WHERE idReclamation = '$idrec'";
            $result = mysqli_query($conn1, $updateQuery);
    
            if (isset($_POST['valider']))
            {
                $updateQuery = "UPDATE `reclamation` SET valider = 'valider' WHERE idReclamation = '$idrec'";
                $result = mysqli_query($conn1, $updateQuery);
                $req2 = "INSERT INTO notification(etat,envoyeur,type,recepteur) VALUES ('0','Q0979','valider reclamation', '$mat')";
                ...
                $updateQuery = "UPDATE `reclamation` SET valider = 'refuser' WHERE idReclamation = '$idrec'";
                $result = mysqli_query($conn1, $updateQuery);
                $req2 = "INSERT INTO notification(etat,envoyeur,type,recepteur) VALUES ('0','Q0979','refuer reclamation', '$mat')";
                $res2 = $conn1->query( $req2);
                header("location:consulterReclamation.php");
            }
        }
...
<a href="consulterReclamation.php"> <button class="button" type="button" >Annuler</button></a> 
      </div></div>
<style>
